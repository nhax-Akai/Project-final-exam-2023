...
Adam là một thuật toán tối ưu hóa phổ biến trong huấn luyện mạng nơ-ron để cải thiện hiệu quả và tốc độ hội tụ [6].  
...
Ngoài ra, Adam còn áp dụng kỹ thuật chỉnh sửa sai số ban đầu của trung bình trượt để cải thiện hiệu năng [6]. Adam phổ biến nhờ khả năng hội tụ nhanh [8], xử lý tốt gradient thưa thớt & nhiễu [7], đồng thời không đòi hỏi nhiều siêu tham số như một số thuật toán khác [9].  

### Root Mean Square Propagation(RMSProp)
RMSProp là một thuật toán tối ưu hóa được sử dụng trong học máy và học sâu để tối ưu hóa quá trình huấn luyện mạng nơ-ron [10].
...
| --- | --- | --- |
| SGD | - Dễ dàng thực hiện và tính toán hiệu quả.<br/>- Hiệu quả đối với các tập dữ liệu lớn với không gian đặc trưng nhiều chiều. | - SGD có thể bị mắc kẹt trong các cực tiểu cục bộ.  <br/>- Độ nhạy cao với tốc độ học ban đầu |
| Adam | - Hiệu quả và dễ thực hiện. <br/>- Áp dụng cho các tập dữ liệu lớn và mô hình nhiều chiều. <br/>- Khả năng khái quát hóa tốt. | - Cần phải điều chỉnh cẩn thận các hyperparameter. |
...
### Hình ảnh minh họa
![markdown](https://github.com/nhax-Akai/Project-final-exam-2023/blob/main/386889797_371110448658396_9011251296850470927_n.png)
...
1.  Initial training - huấn luyện mô hình trên tập dữ liệu ban đầu. Mô hình học một tập các tham số khởi đầu dựa trên các mẫu mà nó nhận thấy trong dữ liệu.
2.  Deployment - mô hình được sử dụng để thực hiện nhiệm vụ dự định. Trong thời gian này, dữ liệu mới liên quan đến nhiệm vụ và môi trường được thu thập.
3.  Data rehearsal - mô hình được điều chỉnh thường xuyên bằng cách ôn lại các kinh nghiệm trước đó, để không quên các thông tin đã học trước đó, trong khi được huấn luyện dữ liệu mới.
4.  Continuous learning strategy - một chiến lược học tập liên tục được áp dụng để thích ứng và cải thiện hiệu suất mô hình.
5.  Revaluation and monitoring - hiệu suất mô hình được đánh giá lặp đi lặp lại về độ chính xác, khả thi, hành vi thực tế và độ chệch.
...
#### Khái niệm: 
Test Production là quá trình kiểm tra đánh giá mô hình học máy sau khi đã huấn luyện xong trên tập dữ liệu thực tế. Mục đích là xác định xem mô hình đó có thể hoạt động tốt và đáp ứng yêu cầu nghiệp vụ hay không khi triển khai trong môi trường thực tế.
